...
from util import format_results, load_predictions, load_gold_data, load_custom_predictions
...
if __name__ == "__main__":
    ...
    # gold_examples = load_predictions(args.prediction_file, args.load_gold)
    # gold_examples = load_gold_data(args.gold_data, args.load_gold)
    pred_examples = load_custom_predictions(args.prediction_file, load_gold=False)
    gold_examples = load_custom_predictions(args.gold_data, load_gold=True)
    ...
    for gold_id in list(gold_examples):
        if gold_id in pred_examples:
            ...
            if gold_example["intent"] == pred_example["intent"].lower():
                correct_intents += 1

            gold_utts = gold_example["intent"] + " " + str(gold_example["entities"])
            pred_utts = pred_example["intent"].lower() + " " + str(pred_example["entities"])

            if gold_utts == pred_utts:
                correct_utts += 1

            intent_f1(gold_example["intent"], pred_example["intent"].lower())
    ...
    print("Intent accuracy: {:.2f}%".format(correct_intents / n_gold_examples * 100))
    print("Utterance accuracy: {:.2f}%".format(correct_utts / n_gold_examples * 100))
